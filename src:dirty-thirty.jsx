            ...
            <div style={{ gridColumn: "1/-1", textAlign: "center", padding: 60, color: C.muted, fontFamily: "'JetBrains Mono'", fontSize: 11 }}>
              {error ? "BACKEND UNAVAILABLE" : (
                <div>
                  ...
                  <div style={{ color: C.muted, marginBottom: 8 }}>KEINE SPIELE TODAY</div>
  ...
  return (
    <div style={{ maxWidth: 980, margin: "0 auto", padding: "24px 20px" }}>
      <div className="fu" style={{ marginBottom: 24 }}>
        ...
        <p style={{ fontFamily: "'JetBrains Mono'", fontSize: 10, color: C.muted, letterSpacing: 2, marginTop: 3 }}>{ranked.length} SPIELER TODAY</p>
      </div>
          ...
          return (
            <div key={entry.userId} className="fu" style={{ animationDelay: `${idx * 0.025}s`, background: isMe ? C.accentDim : C.card, border: `1px solid ${isMe ? C.accent : C.border}`, borderRadius: 10, padding: "13px 18px", display: "flex", alignItems: "center", gap: 14 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontFamily: "'JetBrains Mono'", fontSize: 9, color: C.muted, marginTop: 3 }}>{entry.p1Name ? `${entry.p1Name} (${entry.p1pts ?? "?"}) + ${entry.p2Name ?? "—"} (${entry.p2pts ?? "?"})` : "No picks yet"}</div>
              </div>
              <div style={{ textAlign: "right" }}>
                ...
                <div style={{ fontFamily: "'JetBrains Mono'", fontSize: 9, marginTop: 2, letterSpacing: 1 }}>
                  {isBust ? <span style={{ color: C.red }}>💥 BUST</span> : isPerfect ? <span style={{ color: C.gold }}>🎯 PERFECT</span> : delta !== null ? <span style={{ color: C.muted }}>{delta} FROM 30</span> : <span style={{ color: C.muted }}>PENDING</span>}
                </div>
              </div>
            </div>
          );
        })}
        {ranked.length === 0 && <div style={{ textAlign: "center", padding: 80, color: C.muted, fontFamily: "'JetBrains Mono'", fontSize: 11 }}>NOCH KEINE PICKS — BE THE FIRST!</div>}
      </div>
    </div>
  );
}
